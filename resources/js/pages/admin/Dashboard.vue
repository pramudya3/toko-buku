...
type Props = {
    ...
    lowStockBooks: StockBook[];
    emptyStockBooks: StockBook[];
...
<template>
    ...
    <div class="flex flex-col gap-4 p-4 md:p-6">
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <Card class="p-6">
                ...
                <CardContent class="p-0 pt-3">
                    <p class="text-2xl font-semibold tabular-nums">
                        {{ stats.book_count }}
                        <span class="text-sm font-normal text-muted-foreground">buku</span>
                        · {{ stats.total_stock }}
                        <span class="text-sm font-normal text-muted-foreground">stok</span>
                    </p>
                </CardContent>
            </Card>
        </div>
        ...
        <div class="grid gap-4 lg:grid-cols-3">
            <!-- Grafik penjualan -->
            <Card class="flex h-72 flex-col gap-4 lg:col-span-3">
                <CardHeader class="pb-0">
                    <CardTitle class="text-base font-medium">Penjualan Harian</CardTitle>
                    <div data-slot="card-action">
                        <span class="flex items-center gap-1.5 text-xs text-muted-foreground">
                            <span class="size-2.5 rounded-sm border border-primary/40 bg-primary/15" />
                            Penjualan bersih (setelah refund)
                        </span>
                    </div>
                </CardHeader>
                <CardContent class="flex min-h-0 flex-1 flex-col">
                    <div v-if="salesChart.length" class="flex min-h-0 flex-1 items-end gap-1">
                        <div v-for="point in salesChart" :key="point.date"
                    ...
                    <EmptyState v-else title="Belum ada penjualan"
                        description="Order yang selesai akan muncul di grafik ini." class="flex-1" />

                    <template v-if="salesChart.length">
                        <div class="flex gap-1 pt-2 text-[10px] text-muted-foreground">
                            <span v-for="point in salesChart" :key="point.date" class="flex-1 truncate text-center">
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:col-span-3">
                <Card class="flex h-72 flex-col gap-4">
                    <CardHeader class="pb-0">
                        <CardTitle class="flex items-center gap-2 text-base font-medium">
                            ...
                            <span v-if="lowStockBooks.length"
                        ...
                        <div data-slot="card-action">
                            <Button variant="ghost" size="sm" as-child>
                                <Link :href="booksIndex({ query: { low_stock: '1' } }).url">
                                    Lihat semua
                                </Link>
                            </Button>
                        </div>
                    ...
                    <CardContent class="min-h-0 flex-1 overflow-y-auto">
                        <ul v-if="lowStockBooks.length" class="space-y-2">
                            <li v-for="book in lowStockBooks" :key="book.id"
                                ...
                                <StatusBadge variant="warning" :label="`${book.stok} stok`" />
                            </li>
                        </ul>
                        <EmptyState v-else title="Stok aman" description="Tidak ada buku dengan stok menipis.">
                ...
                <Card class="flex h-72 flex-col gap-4">
                    <CardHeader class="pb-0">
                        ...
                        <div data-slot="card-action">
                            <Button variant="ghost" size="sm" as-child>
                                <Link :href="booksIndex({ query: { low_stock: 'kosong' } }).url">
                                    Lihat semua
                                </Link>
                            </Button>
                        </div>
                    ...
                    <CardContent class="min-h-0 flex-1 overflow-y-auto">
                        <ul v-if="emptyStockBooks.length" class="space-y-2">
                            <li v-for="book in emptyStockBooks" :key="book.id"
                                class="flex items-center justify-between gap-2 rounded-lg border px-3 py-2">
                                <div class="min-w-0">
                                    <p class="truncate text-sm font-medium">
                                        {{ book.judul }}
                        ...
                        <EmptyState v-else title="Stok tersedia" description="Tidak ada buku dengan stok kosong.">
                            <Button variant="outline" size="sm" as-child>
                                <Link :href="booksIndex()">Kelola Buku</Link>
                            </Button>
                        </EmptyState>
        ...
        <Card>
            <CardHeader>
                <CardTitle class="text-base font-medium">Pesanan Terbaru</CardTitle>
                <div data-slot="card-action">
                    <Button variant="ghost" size="sm" as-child>
            ...
            <CardContent class="p-0">
                <Table>
                    ...
                    <TableBody>
                        <TableRow v-for="order in recentOrders" :key="order.id">
                            ...
                            <TableCell>
                                <StatusBadge :variant="statusVariant[order.status] ?? 'neutral'
                                    " :label="statusOptions[order.status] ??
                                        order.status
                                        " />
                            </TableCell>
                ...
                <EmptyState v-if="!recentOrders.length" title="Belum ada pesanan" />
            </CardContent>
        </Card>
    </div>
</template>
