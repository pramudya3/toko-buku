...
/**
 * EditorialLayout — layout halaman artikel storefront utama.
 *
 * Desain editorial Islami: latar hangat, tipografi Inter + Lora, aksen hijau
 * tua & emas lembut, banyak ruang kosong. Header sticky + pencarian artikel
 * (filter beranda), menu mobile, dan footer "Pustaka" yang tenang.
 *
 * Halaman komersial (/buku, checkout, pesanan) tetap memakai CustomerLayout.
 */
...
const tagline = computed(() =>
    String(page.props.tagline ?? 'Pustaka Cahaya Peradaban'),
);
const storeAddress = computed(() =>
    String(page.props.storeAddress ?? ''),
);
const storePhone = computed(() =>
    String(page.props.storePhone ?? ''),
);
...
// Halaman tanpa pencarian: detail buku, checkout, & pesanan — fokus ke
// konten; link "Toko" di nav desktop juga disembunyikan di halaman ini.
...
watch(
    ...
    () => {
        if (!isHomePage.value && !isCatalogPage.value) {
            return;
        }
...
<template>
    <div class="flex min-h-screen flex-col bg-article-bg text-article-ink antialiased">
        ...
        <header
            ...
            <div class="mx-auto flex w-full max-w-6xl flex-col px-4">
                ...
                <div class="hidden h-20 items-center gap-3 lg:flex">
                    <!-- Logo → Beranda -->
                    <Link :href="homeRoute().url" class="flex shrink-0 items-center gap-2.5"
                        :aria-label="`${storeName} — beranda`">
                        <img :src="storeLogoUrl || '/logo-pcp.png'" :alt="storeName"
                            class="h-9 w-auto object-contain" />
                        <span class="hidden text-[17px] font-extrabold tracking-tight lg:inline">{{ storeName }}</span>
                    </Link>

                    <!-- Pencarian (desktop) — selalu terlihat, cari saat Enter -->
                    <div v-if="showHeaderSearch" class="relative ml-2 hidden flex-1 lg:block">
                        <label class="sr-only" for="ed-search">Cari buku atau artikel</label>
                        <Search
                            class="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-article-muted"
                            aria-hidden="true" />
                        <input id="ed-search" v-model="headerSearch" type="search" placeholder="Cari judul / penulis…"
                        ...
                        <button v-if="headerSearch" type="button"
                    ...
                    <!-- Nav desktop -->
                    <nav class="ml-2 hidden shrink-0 items-center gap-1 text-sm lg:flex" aria-label="Navigasi utama">
                        <!-- Link Toko — disembunyikan saat di halaman Toko / detail / checkout -->
                        <Link v-if="!isCatalogPage && !isNoSearchPage" :href="catalogUrl().url"
                    ...
                    <div class="ml-auto flex shrink-0 items-center gap-1">
                        <!-- Keranjang — katalog & detail buku (di checkout sudah di halaman cart) -->
                        <Link v-if="isCatalogPage || isBookDetailPage" :href="'/checkout'"
                        ...
                        <span
                            v-if="isCatalogPage || isBookDetailPage"
                            ...
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end" class="min-w-44">
                                    ...
                                    <DropdownMenuItem class="text-destructive focus:text-destructive" as-child>
                                        <Link :href="logout()" @click="router.flushAll()" as="button" class="w-full">
                ...
                <div class="flex h-16 items-center gap-2 lg:hidden">
                    ...
                    <template v-if="isCatalogPage">
                        <Link :href="homeRoute().url" class="flex min-w-0 shrink-0 items-center"
                            :aria-label="`${storeName} — beranda`">
                            <img :src="storeLogoUrl || '/logo-pcp.png'" :alt="storeName"
                        ...
                        <label class="relative block min-w-0 flex-1">
                            ...
                            <Search
                                class="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-article-muted"
                                aria-hidden="true" />
                            <input v-model="headerSearch" type="search" placeholder="Cari judul / penulis…"
                                class="min-h-10 w-full rounded-lg border border-article-border bg-article-surface py-2 pr-9 pl-9 text-sm transition-colors outline-none placeholder:text-article-muted focus:border-article-primary focus:ring-2 focus:ring-article-primary/20"
                                @keyup.enter="submitSearch" />
                            <button v-if="headerSearch" type="button"
                                class="absolute top-1/2 right-2 flex size-6 -translate-y-1/2 items-center justify-center rounded text-article-muted transition-colors hover:text-article-ink"
                    ...
                    <template v-else-if="isNoSearchPage">
                        <Link :href="homeRoute().url" class="flex min-w-0 shrink-0 items-center gap-2"
                            :aria-label="`${storeName} — beranda`">
                            <img :src="storeLogoUrl || '/logo-pcp.png'" :alt="storeName"
                                class="h-8 w-auto object-contain" />
                            <span class="truncate text-sm font-extrabold tracking-tight">{{ storeName }}</span>
                        </Link>

                        <div class="ml-auto flex shrink-0 items-center">
                            <!-- Cart di kanan — hanya detail buku (checkout sudah di halaman cart) -->
                            <Link v-if="isBookDetailPage" :href="'/checkout'"
                    ...
                    <template v-else>
                        ...
                        <div class="ml-auto flex shrink-0 items-center">
                            ...
                            <button type="button"
                                ...
                                <X v-if="mobileSearchOpen" class="size-5" aria-hidden="true" />
            ...
            <div v-if="mobileSearchOpen && !isCatalogPage" id="ed-mobile-search-row"
                ...
                <div class="mx-auto max-w-6xl px-4 py-3">
                    <label class="relative block">
                        ...
                        <Search
                            class="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-article-muted"
                            aria-hidden="true" />
                        <input id="ed-search-mobile" v-model="headerSearch" type="search"
        ...
        <footer class="border-t border-article-border bg-article-surface pb-18 lg:pb-0">
            ...
            <div class="mx-auto flex max-w-6xl flex-col gap-0.5 px-4 py-6 text-left">
                ...
                <p v-if="storeAddress" class="text-xs text-article-muted">
                    {{ storeAddress }}
                </p>
                <p v-if="storePhone" class="text-xs text-article-muted">
                    {{ storePhone }}
                </p>
            </div>
        ...
        <!-- ══════════ BOTTOM NAV (mobile only) ══════════ -->
        <nav class="fixed inset-x-0 bottom-0 z-40 border-t border-article-border bg-article-surface lg:hidden"
            ...
            <div class="mx-auto flex h-16 max-w-6xl items-center justify-around px-2">
                ...
                <template v-if="user">
                    <DropdownMenu>
                        <DropdownMenuTrigger as-child>
                            <button type="button"
                                ...
                                <span
                                    ...
                                    :class="isProfilActive && 'ring-2 ring-article-primary/30'"
                                >
                                    {{ initials }}
                                </span>
                                <span>Profil</span>
                        ...
                        <DropdownMenuContent align="end" side="top" :side-offset="8" class="min-w-44">
                            <DropdownMenuItem as-child>
                                <Link :href="'/profil'">
                                    Profil Saya
                                </Link>
