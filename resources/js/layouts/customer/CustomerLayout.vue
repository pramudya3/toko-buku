...
import { BookOpen, LogOut, Menu, Package, Settings, ShoppingCart } from '@lucide/vue';
        ...
        <header
            ...
            <div class="mx-auto flex h-14 w-full max-w-6xl items-center gap-2 px-4">
                ...
                <div class="ml-auto flex items-center gap-1">
                    ...
                    <Button v-if="isAdmin" variant="outline" size="sm" class="hidden md:inline-flex" as-child>
                    ...
                    <template v-if="user && !isAdmin">
                        <DropdownMenu>
                            <DropdownMenuTrigger as-child>
                                <Button variant="ghost" class="hidden size-9 rounded-full p-0 md:inline-flex"
                                    title="Menu akun">
                                    <Avatar class="size-8">
                    ...
                    <Sheet v-model:open="mobileMenuOpen">
                        ...
                        <SheetContent side="right" class="w-72">
                            ...
                            <div class="flex flex-col gap-1 px-2 py-4">
                                ...
                                <Link v-if="user && !isAdmin" :href="'/settings/akun'"
                                    class="rounded-md px-3 py-2.5 text-sm font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                                    @click="mobileMenuOpen = false">
                                    Settings
                                </Link>
                            ...
                            <div class="mt-auto flex flex-col gap-2 border-t px-2 pt-4">
                                <template v-if="user">
                                    <p v-if="!isAdmin" class="px-3 text-sm text-muted-foreground">
                                        {{ user.name }}
                                    </p>
                                ...
                                <template v-else>
                                    <Button variant="ghost" size="sm" as-child>
                                        <Link :href="'/login'" @click="mobileMenuOpen = false">Masuk</Link>
                                    </Button>
